...
from gedi.backbones.pointnet2_ops_lib.pointnet2_ops.pointnet2_modules import (
    PointnetSAModule, )
...
class tnet(nn.Module):
    ...
    def __init__(self, ):
        ...
        self.conv1 = nn.Sequential(nn.Conv1d(3, 256, 1, bias=False), nn.BatchNorm1d(256), nn.ReLU())

        self.conv2 = nn.Sequential(nn.Conv1d(256, 512, 1, bias=False), nn.BatchNorm1d(512),
                                   nn.ReLU())

        self.conv3 = nn.Sequential(nn.Conv1d(512, 1024, 1, bias=False), nn.BatchNorm1d(1024))

        self.fc1 = nn.Sequential(nn.Linear(1024, 512, bias=False), nn.BatchNorm1d(512), nn.ReLU())

        self.fc2 = nn.Sequential(nn.Linear(512, 256, bias=False), nn.BatchNorm1d(256), nn.ReLU())
...
class PointNet2Feature(nn.Module):
    ...
    def __init__(self, dim=32):
        ...
        self.samodule3 = PointnetSAModule(mlp=[256 + 3, 512, 512, 1024], use_xyz=self.use_xyz)
...
class GeDi:
    ...
    def __init__(self, config):
        ...
        self.gedi_net.load_state_dict(
            torch.load(config['fchkpt_gedi_net'])['pnet_model_state_dict'])
    ...
    def compute(self, pts, pcd):
        ...
        out = ml3d.ops.radius_search(pcd,
                                     pts,
                                     radii,
        ...
        iter_num = int(np.ceil(len(pts) / self.samples_per_batch))
        for b in trange(iter_num, desc='[GeDi] Computing descriptors'):
