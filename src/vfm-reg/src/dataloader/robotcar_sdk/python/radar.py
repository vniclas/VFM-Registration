...
def load_radar(
        example_path: AnyStr) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]:
    ...
    azimuths = (raw_example_data[:, 8:10].copy().view(np.uint16) / float(encoder_size) * 2 *
...
def radar_polar_to_cartesian(azimuths: np.ndarray,
                             fft_data: np.ndarray,
