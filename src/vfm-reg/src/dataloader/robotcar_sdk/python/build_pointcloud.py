...
from velodyne import (
    load_velodyne_binary,
    load_velodyne_raw,
    velodyne_raw_to_pointcloud,
)
    ...
    if poses_type in ['ins', 'rtk']:
        with open(os.path.join(extrinsics_dir, 'ins.txt')) as extrinsics_file:
            ...
            G_posesource_laser = np.linalg.solve(
                build_se3_transform([float(x) for x in extrinsics.split(' ')]), G_posesource_laser)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--extrinsics_dir',
                        type=str,
                        default=None,
                        help='Directory containing extrinsic calibrations')
    parser.add_argument('--laser_dir',
                        type=str,
                        default=None,
    ...
    lidar = re.search('(lms_front|lms_rear|ldmrs|velodyne_left|velodyne_right)',
                      args.laser_dir).group(0)
