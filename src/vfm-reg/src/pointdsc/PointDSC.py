...
from pointdsc.common import knn, rigid_transform_3d
from pointdsc.SE3 import transform
...
class NonLocalBlock(nn.Module):
    ...
    def forward(self, feat, attention):
        ...
        feat_attention = torch.einsum('bhco, bhci->bhoi', Q,
                                      K) / (self.num_channels // self.head)**0.5
...
class PointDSC(nn.Module):
    ...
    def forward(self, data):
        ...
        with torch.no_grad():
            ...
            corr_compatibility = src_dist - torch.norm(
                (tgt_keypts[:, :, None, :] - tgt_keypts[:, None, :, :]), dim=-1)
            corr_compatibility = torch.clamp(1.0 - corr_compatibility**2 / self.sigma_spat**2,
                                             min=0)
        ...
        if testing:
            seeds = self.pick_seeds(src_dist,
                                    confidence,
                                    R=self.nms_radius,
                                    max_num=int(num_corr * self.ratio))
        else:
            seeds = torch.argsort(confidence, dim=1, descending=True)[:,
                                                                      0:int(num_corr * self.ratio)]

        #################################
        # Step 3 & 4: calculate transformation matrix for each seed, and find the best hypothesis.
        #################################
        seed_trans, seed_fitness, final_trans, final_labels = self.cal_seed_trans(
            seeds, normed_corr_features, src_keypts, tgt_keypts)
    ...
    def cal_seed_trans(self, seeds, corr_features, src_keypts, tgt_keypts):
        ...
        """
        bs, num_corr, num_channels = corr_features.shape[0], corr_features.shape[
            1], corr_features.shape[2]
        ...
        knn_idx = knn_idx.gather(dim=1, index=seeds[:, :, None].expand(-1, -1,
                                                                       k))  # [bs, num_seeds, k]

        #################################
        # construct the feature consistency matrix of each correspondence subset.
        #################################
        knn_features = corr_features.gather(
            dim=1, index=knn_idx.view([bs, -1])[:, :, None].expand(-1, -1, num_channels)).view(
        ...
        #################################
        src_knn = src_keypts.gather(dim=1,
                                    index=knn_idx.view([bs, -1])[:, :, None].expand(
                                        -1, -1, 3)).view([bs, -1, k, 3])  # [bs, num_seeds, k, 3]
        tgt_knn = tgt_keypts.gather(dim=1,
                                    index=knn_idx.view([bs,
                                                        -1])[:, :,
                                                             None].expand(-1, -1,
                                                                          3)).view([bs, -1, k, 3])
        ...
        if seed_as_center:
            ...
            src_center = src_keypts.gather(dim=1, index=seeds[:, :,
                                                              None].expand(-1, -1,
                                                                           3))  # [bs, num_seeds, 3]
            tgt_center = tgt_keypts.gather(dim=1, index=seeds[:, :,
                                                              None].expand(-1, -1,
                                                                           3))  # [bs, num_seeds, 3]
            src_center, tgt_center = src_center.view([-1, 3]), tgt_center.view([-1, 3])
            src_pts = src_knn[:, :, :, None] - src_center[:, None, :,
                                                          None]  # [bs*num_seeds, k, 3, 1]
            tgt_pts = tgt_knn[:, :, :, None] - tgt_center[:, None, :,
                                                          None]  # [bs*num_seeds, k, 3, 1]
            cov = torch.einsum('nkmo,nkop->nkmp', src_pts,
                               tgt_pts.permute(0, 1, 3, 2))  # [bs*num_seeds, k, 3, 3]
            ...
            t = tgt_center[:, None, :] - src_center[:, None, :] @ R.permute(0, 2,
                                                                            1)  # [num_pair, 1, 3]
        ...
        #################################
        pred_position = torch.einsum(
            'bsnm,bmk->bsnk', seedwise_trans[:, :, :3, :3], src_keypts.permute(
                0, 2, 1)) + seedwise_trans[:, :, :3, 3:4]  # [bs, num_seeds, num_corr, 3]
        pred_position = pred_position.permute(0, 1, 3, 2)
        L2_dis = torch.norm(pred_position - tgt_keypts[:, None, :, :],
                            dim=-1)  # [bs, num_seeds, num_corr]
        seedwise_fitness = torch.mean((L2_dis < self.inlier_threshold).float(),
                                      dim=-1)  # [bs, num_seeds]
        ...
        final_trans = seedwise_trans.gather(dim=1,
                                            index=batch_best_guess[:, None, None,
                                                                   None].expand(-1, -1, 4,
                                                                                4)).squeeze(1)
        final_labels = L2_dis.gather(dim=1,
                                     index=batch_best_guess[:, None, None].expand(
                                         -1, -1, L2_dis.shape[2])).squeeze(1)
