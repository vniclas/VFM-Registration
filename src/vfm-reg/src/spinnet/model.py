...
import spinnet.common as cm
import spinnet.ThreeDCCN as pn
import torch
import torch.nn as nn
import torch.nn.functional as F
from spinnet.common import switch
    ...
    def forward(self, input):
        ...
        for case in switch(self.dataset):
            if case('3DMatch'):
                ...
                R = cm.RodsRotatFormula(
                    z_axis,
                    torch.FloatTensor([0, 0, 1]).unsqueeze(0).repeat(z_axis.shape[0], 1))
        ...
        S2_xyz = torch.FloatTensor(
            cm.get_voxel_coordinate(radius=self.des_r,
                                    rad_n=self.rad_n,
                                    azi_n=self.azi_n,
                                    ele_n=self.ele_n))
        ...
        new_points = cm.sphere_query(delta_x,
                                     pts_xyz,
                                     radius=self.voxel_r,
