...
def select_patches(pts,
    ...
    for i in range(np.size(ind_local)):
        ...
        if local_neighbors.shape[0] >= num_points_per_patch:
            temp = np.random.choice(range(local_neighbors.shape[0]),
            ...
            while local_neighbors.shape[0] + fix_idx.shape[0] < num_points_per_patch:
                fix_idx = np.concatenate((fix_idx, np.asarray(range(local_neighbors.shape[0]))),
                                         axis=0)
            random_idx = np.random.choice(local_neighbors.shape[0],
                                          num_points_per_patch - fix_idx.shape[0],
                                          replace=False)
...
def pc_rotate_translate_torch(data, angles, translates):
    ...
    rotated_data = torch.matmul(data, R.transpose(
        -1, -2)) + torch.FloatTensor(translates).unsqueeze(1).to(device)
...
def _pc_rotate_translate_torch(data, R, translates):
    ...
    rotated_data = torch.matmul(data,
                                R.to(device).transpose(
                                    -1, -2)) + torch.FloatTensor(translates).unsqueeze(1).to(device)
...
def pad_image(input, kernel_size):
    ...
    if kernel_size % 2 == 0:
        ...
        zeros_pad = torch.zeros([output.shape[0], output.shape[1], pad_size,
                                 output.shape[3]]).to(device)
...
def pad_image_3d(input, kernel_size):
    ...
    if kernel_size % 2 == 0:
        ...
        zeros_pad = torch.zeros(
            [output.shape[0], output.shape[1], output.shape[2], pad_size,
             output.shape[4]]).to(device)
...
def RodsRotatFormula(a, b):
    ...
    a33 = zero
    Rx = torch.cat((torch.cat((a11, a12, a13), dim=2), torch.cat(
        (a21, a22, a23), dim=2), torch.cat((a31, a32, a33), dim=2)),
                   dim=1)
    I = torch.eye(3).to(device)
    R = I.unsqueeze(0).repeat(
        B, 1, 1) + torch.sin(theta) * Rx + (1 - torch.cos(theta)) * torch.matmul(Rx, Rx)
...
def collect_local_neighbor(ref_pcd,
                           pcd,
                           vicinity=0.3,
                           num_points_per_patch=1024,
                           random_state=None):
